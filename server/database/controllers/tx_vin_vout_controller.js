...
function countTxForBlock(blockindex, cb) {
    TxVinVout[db.getCurrentConnection()].find({blockindex: blockindex}).countDocuments({}, function (err, count) {
        if(err) {
            cb()
        } else {
            cb(count);
            ...
            this.getFirstByOrder(blockindex, sortOposite, (tx) => {
                if(tx) {
                    this.countTxForBlock(blockindex, function (count) {
                        ...
                        if (offset) {
                            // aggregate.push({$skip: offset * limit});
                            // aggregate.push({$match: {order: {$lte: tx.order - 1 + (count - offset * limit)}}});
                            var from = tx.order + (offset * limit);
                            var to = from + limit;
                            aggregate.push({$match: {order: {$gte: from, $lt: to}}});
                        }
                        aggregate.push({$limit: limit});
                        aggregate.push({
                            "$unwind": {
