...
var commands_require_db = [
    ...
    'save_tx_vin_vout_and_addresses_cursor',
    'reindex_addresses',

    'update_tx',
    'fix_tx_from_to',
    'update_tx_vin_vout_and_addresses',
    'update_tx_vin_vout_and_addresses_cursor',
    ...
    switch(type)
    {
        ...
        case 'getnetworkhashps':
            wallet_commands.getNetworkHashps(wallet).then(function(networkhashps){
                ...
                console.log('error getting networkhashps', err);
                process.exit();
            });
            break;
        case 'getinfo':
        ...
        case 'save_tx_vin_vout_and_addresses': // 1:8:42.847 - block count 388282
            ...
            break;
        case 'save_tx_vin_vout_and_addresses_cursor': // 1:8:42.847 - block count 388282
            if (cluster.isMaster) {
                var startTime = new Date();
                var countAddress = 0;
                console.log(`Master ${process.pid} is running`);
                    ...
                    var startReIndexClusterLinerAll = function() {
                        TxController.getAll('blockindex', 'desc', 1, function(latestTx) {
                            ...
                            if(latestTx.length) {
                                ...
                                console.log('no blocks found - please run save_tx first');
                                deleteFile();
                                db.multipleDisconnect();
                                process.exit();
                                return;
        ...
        case 'update_tx_vin_vout_and_addresses': // 12:47:25.775 - block count 268159
            ...
            break;
        case 'update_tx_vin_vout_and_addresses_cursor': // 12:47:25.775 - block count 268159
            if (cluster.isMaster) {
                var startTime = new Date();
                // console.log(`Master ${process.pid} is running`);
                if(fileExist()) {
